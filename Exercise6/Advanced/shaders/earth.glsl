...
void main() {
    ...
    vec3 color;
    {
        ...
        if(useClouds)
        {
            ...
            float clouds = 0.5f;
            dayColor *=  clouds;
            // <-- modify this line with the specular intensity value

		}

                vec3 color_diffuse = sunColor * dayColor * mix(nightColor, dayColor, dot(n, l)); // <- change this line for 6.5a)
		...
		// The texture stores values between 0 and 1. Scale these values to [0, 0.7] and 
                // then clamp values smaller than 0.2 to dayColor0.2 to obtain a natural look.
        vec3 specular = vec3(texture(earthSpec, tc)) * 0.7;

        if (specular[0] < 0.2 || specular[1] < 0.2 || specular[2] < 0.2){
            specular = dayColor * 0.2;
        }

        vec3 color_specular = sunColor * pow(max(0.0, dot(v, r)), 20) * specular;//  (vec3(texture(earthSpec, tc)) * 0.7); // <-- modify this line with the specular intensity value
