        ...
        vec3 n, vec3 l, vec3 v)
{
    ...
    color_ambient = surfaceColor * light.color * light.ambientIntensity;
    color_diffuse = surfaceColor * light.color * light.diffuseIntensity * dot(n, l);

    vec3 r = normalize(2. * dot(n, l) * n - l);
    float dot_vr = dot(v, r);
    color_specular = surfaceColor * light.specularIntensity * pow(dot_vr, 2.);
...
void main()
{
    ...
    if(directionalLight.enable)
    {
        ...
        colorDirectional = phong(directionalLight, objectColor, n, l, v); //<- change this line
    ...
    if(pointLight.enable)
    {
        ...
        vec3 I0 = phong(pointLight, objectColor, n, l, v); //<- change this line

        float a = pointLight.attenuation[0];
        float b = pointLight.attenuation[1];
        float c = pointLight.attenuation[2];
        float r = distance( positionWorldSpace, pointLight.position);
        colorPoint = I0 / (a + b*r + c*r*r);
    ...
    if(spotLight.enable)
    {
        ...
        vec3 l = - spotLight.direction;
        vec3 I0 = phong(spotLight, objectColor, n, l, v); //<- change this line

        float a = spotLight.attenuation[0];
        float b = spotLight.attenuation[1];
        float c = spotLight.attenuation[2];

        float r = distance(spotLight.position,positionWorldSpace);

        float coneAngle = acos(dot(spotLight.direction, normalize(positionWorldSpace - spotLight.position)));

        colorSpot = I0 / (a + b*r + c*r*r);
        colorSpot *= smoothstep(spotLight.angle, (spotLight.angle * spotLight.sharpness - 0.0001), coneAngle);

        if(coneAngle > spotLight.angle){
            colorSpot = vec3(0);
        }
    ...
    {
        //TODO 5.4 e)
        vec3 colorDirectionalHSV = rgb2hsv(colorDirectional);
        vec3 colorPointHSV = rgb2hsv(colorPoint);
        vec3 colorSpotHSV = rgb2hsv(colorSpot);


        colorDirectionalHSV[2] = floor(colorDirectionalHSV[2]*3.)/2.;
        colorPointHSV[2] = floor(colorPointHSV[2]*3.)/2.;
        colorSpotHSV[2] = floor(colorSpotHSV[2]*3.)/2.;

        colorDirectional = hsv2rgb(colorDirectionalHSV);
        colorPoint = hsv2rgb(colorPointHSV);
        colorSpot = hsv2rgb(colorSpotHSV);
