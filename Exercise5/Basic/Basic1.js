...
function PhongLighting(context, point, normal, eye, pointLight, albedo, showVectors) {
    ...
    vec2.normalize(l, l);
    var cos_nl = normal[0]*l[0] + normal[1]*l[1];
    var r = vec2.fromValues(2 * cos_nl * normal[0] - l[0], 2 * cos_nl * normal[1] - l[1]);
    vec2.normalize(r, r);
    var cos_vr = r[0]*v[0] + r[1]*v[1];
    ...
    var L_amb = vec3.fromValues(0.1 * albedo[0], 0.1 * albedo[1], 0.1 * albedo[2]);
    ...
    var L_dif = vec3.fromValues(0.5 * albedo[0] * cos_nl, 0.5 * albedo[1] * cos_nl, 0.5 * albedo[2] * cos_nl);
    ...
    var shiny = 30.0;
    var L_spe = vec3.fromValues(0.4 * Math.pow(cos_vr, shiny), 0.4 * Math.pow(cos_vr, shiny), 0.4 * Math.pow(cos_vr, shiny));

    // 5. Add ambient, diffuse and specular color.
    //    Store the result in the variable color - replace the following dummy line:
    var color = vec3.create();
    color[0] = L_amb[0] + L_dif[0] + L_spe[0];
    color[1] = L_amb[1] + L_dif[1] + L_spe[1];
    color[2] = L_amb[2] + L_dif[2] + L_spe[2];
...
var Basic1_2 = function () {
    ...
    function Render() {
        ...
        for (var i = 0; i < nLineSegments; ++i) {
            ...
            var mid = vec2.fromValues((lineSegments[i][1][1] + lineSegments[i][0][1]) / 2, (lineSegments[i][1][0] + lineSegments[i][0][0]) / 2);
            // 2. Compute the normal of the line segment.
            var n = vec2.fromValues(-(lineSegments[i][1][0] - lineSegments[i][0][0]), (lineSegments[i][1][1] - lineSegments[i][0][1]));
            vec2.normalize(n,n);
            // 3. Use the function PhongLighting that you implemented in the previous assignment to evaluate the color.
            var color = PhongLighting(context, mid, n, eye, pointLight, albedo, false);
...
var Basic1_3 = function () {
    ...
    function Render() {
        ...
        for (var i = 0; i < nLineSegments; ++i) {
            ...
            var start = vec2.fromValues(lineSegments[i][0][1], lineSegments[i][0][0]);
            var end = vec2.fromValues(lineSegments[i][1][1], lineSegments[i][1][0]);
            var n = vec2.fromValues(-(lineSegments[i][1][0] - lineSegments[i][0][0]), (lineSegments[i][1][1] - lineSegments[i][0][1]));
            vec2.normalize(n,n);
            var color_s = PhongLighting(context, start, n, eye, pointLight, albedo, false);
            var color_e = PhongLighting(context, end, n, eye, pointLight, albedo, false);
            ...
            var colorfs = floatToColor(color_s);
            var colorfe = floatToColor(color_e);
            var my_gradient = context.createLinearGradient(start[0], start[1], end[0], end[1]);
            my_gradient.addColorStop(0, 'rgb(' + colorfs[0] + ', ' + colorfs[1] + ', ' + colorfs[2] + ')');
            my_gradient.addColorStop(1, 'rgb(' + colorfe[0] + ', ' + colorfe[1] + ', ' + colorfe[2] + ')');
