...
mat4.perspective = function (out, fovy, near, far) {
    ...
    out[0] = 1.0 / Math.tan(fovy/2);
    out[1] = 0.0;
    out[2] = 0.0;
    out[3] = 0.0;
    
    out[4] = 0.0;
    out[5] = 1.0 / Math.tan(fovy/2);
    out[6] = 0.0;
    out[7] = 0.0;
...
Camera3D.prototype.update = function () {
    ...
    var g = vec3.create();
    g[0] = this.eye[0] - this.lookAtPoint[0];
    g[1] = this.eye[1] - this.lookAtPoint[1];
    g[2] = this.eye[2] - this.lookAtPoint[2];
    vec3.normalize(g, g);
    
    this.w = - g / vec3.length(g);

    var cro = vec3.create();
    vec3.cross(cro, this.upVector, this.w);
    this.u = cro / vec3.length(cro);

    this.v = vec3.create();
    vec3.cross(this.v, this.w, this.u);

    var uTe = this.u[0] * this.eye[0] + this.u[1] * this.eye[1] + this.u[2] * this.eye[2];
    var vTe = this.v[0] * this.eye[0] + this.v[1] * this.eye[1] + this.v[2] * this.eye[2];
    var wTe = this.w[0] * this.eye[0] + this.w[1] * this.eye[1] + this.w[2] * this.eye[2];

    this.cameraMatrix = mat4.fromValues(this.u[0], this.v[0], this.w[0], 0.0, this.u[1], this.v[1], this.w[1], 0.0, this.u[2], this.v[2], this.w[2], 0.0, -uTe, -vTe, -wTe, 1.0);
    // this.cameraMatrix[0] = this.u[0];
    // this.cameraMatrix[1] = this.v[0];
    // this.cameraMatrix[2] = this.w[0];
    // this.cameraMatrix[3] = 0.0;
    
    // this.cameraMatrix[4] = this.u[1];
    // this.cameraMatrix[5] = this.v[1];
    // this.cameraMatrix[6] = this.w[1];
    // this.cameraMatrix[7] = 0.0;
    
    // this.cameraMatrix[8] = this.u[2];
    // this.cameraMatrix[9] = this.v[2];
    // this.cameraMatrix[10] = this.w[2];
    // this.cameraMatrix[11] = 0.0;
   
    // this.cameraMatrix[12] = -uTe;
    // this.cameraMatrix[13] = -vTe;
    // this.cameraMatrix[14] = -wTe;
