var Basic3 = function () {
    ...
    function doAlphaBlending(index, images, alphas) {
        ...
        var alphaB = 1; //alpha background = 0 = initial alpha

        // 2. Loop over the four circle images in the correct order to blend
        //    them one after another.
        for (var i = 0; i < 3; i++) {
            images[i][index] = (1 - alphas[i]) * color[0] * alphaB + alphas[i] * images[i][index];
            images[i][index + 1] = (1 - alphas[i]) * color[1] * alphaB + alphas[i] * images[i][index + 1];
            images[i][index + 2] = (1 - alphas[i]) * color[2] * alphaB + alphas[i] * images[i][index + 2];
            images[i][index + 3] = (1 - alphas[i]) * color[3] * alphaB + alphas[i] * images[i][index + 3];;
            alphaB = alphas[i] + (1-alphas[i])*alphaB;
            color = [images[i][index], images[i][index + 1], images[i][index + 2]];
        }
