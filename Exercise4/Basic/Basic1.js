...
var Basic1_2 = function () {
    ...
    function PerspectiveProjection2D(eye, imagePlane, point2D) {
        ...
        return (point2D[0] - eye[0]) * (imagePlane - eye[1]) / (point2D[1] - eye[1]);
...
Camera.prototype.update = function () {
    ...
    var alpha = Math.atan((this.lookAtPoint[0]-this.eye[0]) / (this.lookAtPoint[1]-this.eye[1]));
    
    this.cameraMatrix[0] = Math.cos(alpha + Math.PI);
    this.cameraMatrix[1] = Math.sin(alpha + Math.PI);
    this.cameraMatrix[2] = 0;

    this.cameraMatrix[3] = -Math.sin(alpha + Math.PI);
    this.cameraMatrix[4] = Math.cos(alpha + Math.PI);
    this.cameraMatrix[5] = 0;

    this.cameraMatrix[6] = - this.eye[0] * Math.cos(alpha + Math.PI) + this.eye[1] * Math.sin(alpha + Math.PI);
    this.cameraMatrix[7] = - this.eye[0] * Math.sin(alpha + Math.PI) - this.eye[1] * Math.cos(alpha + Math.PI);
...
Camera.prototype.projectPoint = function (point2D) {
    ...
    //              necessary.
    var pointHomo = mat3.create();
    pointHomo[0] = point2D[0];
    pointHomo[4] = point2D[1];
    pointHomo[8] = 1.0;
    var product = mat3.create();
    mat3.multiply(product, this.projectionMatrix, this.cameraMatrix);
    mat3.multiply(pointHomo, product, pointHomo);
    return [pointHomo[0] / pointHomo[8] , pointHomo[4] / pointHomo[8]];
}
