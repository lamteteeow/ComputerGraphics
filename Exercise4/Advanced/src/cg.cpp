...
void CG::update(float dt)
{
    ...
    for(Particle& p : particles)
    {
        ...
        if(p.lifeTime < 0){
            p.position = particleStart;
            p.lifeTime = glm::linearRand(0.0, 4.0);
            p.timeOffset = glm::linearRand(0.0, 2.0);
            p.velocity = (glm::linearRand(glm::vec3(0.0), glm::vec3(0.5)) + planeNormal) * glm::linearRand(0.0,2.0);
        }
...
void CG::renderParticles()
{
    ...
    for(Particle& p : particles)
    {
        ...
        float radians = 90 * M_PI / 180.0;
        mat4 particleTransformation =  glm::translate(p.position) * transpose(mat4(mat3(camera.getViewMatrix()))); // <- Change this line
        glm::rotate(particleTransformation, radians, myRotationAxis);
...
static mat3 orthonormalBasis(vec3 dir)
{
    ...
    vec3 u = glm::linearRand(glm::vec3(1), glm::vec3(2));
    v[0] = u;
    v[1] = glm::cross(normalize(dir), normalize(u));
    v[2] = -dir;
    return v;
}
