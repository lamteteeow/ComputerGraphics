...
RenderingPipeline.prototype.PrimitiveAssemblyStage = function (vertexStream, ibo) {
    ...
    var primitives = new Array(vertexStream.length * (vertexStream.length - 1) / 2); // Also change the size of this array.
    var count = 0;
    for (var i = 0; i < primitives.length; i++) {
        for (var j = 0; j < ibo.length; j += 2) {
            primitives[i] = [ vertexStream[ibo[j]], vertexStream[ibo[j + 1]] ];
        }
    }
...
RenderingPipeline.prototype.LineCulling = function (a, b) {
    // a = [x,z,w],  b = [x,z,w]
    var line = [b[0] - a[0], b[1] - a[1], b[2] - a[2]];
