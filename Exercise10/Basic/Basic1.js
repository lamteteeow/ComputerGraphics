...
function Object(primitives) {
    ...
    for (var i = 0; i < this.primitives.length; i++) {
        if(this.primitives[i].p0[0] < xmin){
            xmin = this.primitives[i].p0[0];
        }
        
        if(this.primitives[i].p0[0] > xmax){
            xmax = this.primitives[i].p0[0];
        }
        
        if(this.primitives[i].p0[1] < ymin){
            ymin = this.primitives[i].p0[1];
        }
        
        if(this.primitives[i].p0[1] > ymax){
            ymax = this.primitives[i].p0[1];
        }
    }

    this.aabb =[[xmin, ymax], [xmax, ymin]]; // should be in this format: [[x, y], [x, y]] - [bottom left corner, top right corner]
    ...
    lines[0] = new Line([xmin, ymax], [xmin, ymin], color); 
    lines[1] = new Line([xmin, ymin], [xmax, ymin], color); 
    lines[2] = new Line([xmax, ymin], [xmax, ymax], color); 
    lines[3] = new Line([xmax, ymax], [xmin, ymax], color); 
     this.aabb_primitives = lines;    
}
...
function KDTree(objects) {
    ...
    while (stack.length != 0) {
        ...
        if (node.objects.length > 3) { // The node needs to be split.
            ...
            //    position!
            if (node.splitAxis == 'x') {
                var sorted = sort_along_x(node.objects)
                if (node.objects.length % 2 == 0){ // number of objects is even

                    node.splitPosition = 
                    (sorted[(node.objects.length  / 2)-1].aabb_primitives[3].p0[0] + 
                    sorted[(node.objects.length  / 2)].aabb_primitives[0].p0[0]) 
                    / 2;
                } else {
                    console.log(node.objects.length -1 );
                    node.splitPosition = 
                    (sorted[((node.objects.length-1 )/ 2)-1].aabb_primitives[3].p0[0] + 
                    sorted[((node.objects.length-1 )/ 2)].aabb_primitives[0].p0[0]) 
                    / 2;
                }
                // ...
            } else {
                var sorted = sort_along_y(node.objects)
                if (node.objects.length % 2 == 0){ // number of objects is even

                    node.splitPosition = 
                    (sorted[(node.objects.length / 2)-1].aabb_primitives[0].p0[1] + 
                    sorted[(node.objects.length / 2)].aabb_primitives[1].p0[1]) 
                    / 2;

                } else {
                    node.splitPosition = 
                    (sorted[((node.objects.length-1 )/ 2)-1].aabb_primitives[0].p0[1] + 
                    sorted[((node.objects.length-1) / 2)].aabb_primitives[1].p0[1]) 
                    / 2;
                }
            ...
            for (var i = 0; i < node.objects.length; i++) {
                ...
                if (node.splitAxis == 'x') {
                    if (node.objects[i].aabb_primitives[0].p0[0] < node.splitPosition){
                        objectsLeft.push(node.objects[i]);
                    } else {
                        objectsRight.push(node.objects[i]);
                    }
                        
                } else {
                    if (node.objects[i].aabb_primitives[0].p0[1] < node.splitPosition){
                        objectsLeft.push(node.objects[i]);
                    } else {
                        objectsRight.push(node.objects[i]);
                    }
                }
                console.log(objectsLeft);
            }
            node.objects = null;
            ...
            if (node.splitAxis == 'x') {
                var splitAxis = 'y';
                var leftMax = [node.splitPosition, node.max[1]];
                var leftMin = node.min;
                var rightMax = node.max;
                var rightMin = [node.splitPosition, node.min[1]];
                // ...
            } else {
                var splitAxis = 'x';
                var leftMax = node.max;
                var leftMin =  [node.min[0], node.splitPosition];
                var rightMax = [node.max[0], node.splitPosition];
                var rightMin = node.min;
                // ...
            }
            leftChild = new Node(false, objectsLeft, leftMin, leftMax, splitAxis);
            rightChild = new Node(false, objectsRight, rightMin, rightMax, splitAxis);
